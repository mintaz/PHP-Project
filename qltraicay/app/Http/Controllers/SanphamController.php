...
class SanphamController extends Controller
{
    ...
    {   
		//kiem tra don hang
		$donhang_sp = DB::table('chitietdonhang')->select('*')->where('sanpham_id',$id)->get();
        foreach ($donhang_sp as $key => $val) {
            $khmai[] = $val->donhang_id;
        }
		if (!empty($khmai)) {
			return redirect()->route('admin.sanpham.list')->with(['flash_level'=>'success','flash_message'=>'Sản phẩm có đơn hàng và khách hàng không thể xóa!!!']);
        }
		//xoa don hang
        ...
        return redirect()->route('admin.sanpham.list')->with(['flash_level'=>'success','flash_message'=>'Xóa sản phẩm và dữ liệu liên quan thành công!!!']);
    }
