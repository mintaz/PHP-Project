...
class NhomController extends Controller
{
	...
	{
		//kiem tra loai thuc pham
		$loaisanpham = DB::table('loaisanpham')->select('id')->where('nhom_id',$id)->get();
        foreach ($loaisanpham as $key => $val) {
            $khmai[] = $val->id;
        }
		if (!empty($khmai)) {
			return redirect()->route('admin.nhom.list')->with(['flash_level'=>'success','flash_message'=>'Có Loại sản phẩm thuộc nhóm này không thể xóa!!!']);
        }
        $nhom = DB::table('nhom')->where('id',$id)->first();
