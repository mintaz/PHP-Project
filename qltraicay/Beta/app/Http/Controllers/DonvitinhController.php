...
class DonvitinhController extends Controller
{
    ...
    public function getDelete($id)
    {
		...
		$sanpham = DB::table('sanpham')->select('id')->where('donvitinh_id',$id)->get();
        foreach ($sanpham as $key => $val) {
            $khmai[] = $val->id;
        }
		if (!empty($khmai)) {
			return redirect()->route('admin.donvitinh.list')->with(['flash_level'=>'success','flash_message'=>'Có sản phẩm thuộc đơn vị tính này không thể xóa!!!']);
        }
