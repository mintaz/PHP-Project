...
class NhacungcapController extends Controller
{
    ...
    public function getDelete($id)
    {
		...
		$lohang = DB::table('lohang')->select('id')->where('nhacungcap_id',$id)->get();
        foreach ($lohang as $key => $val) {
            $khmai[] = $val->id;
        }
		if (!empty($khmai)) {
			return redirect()->route('admin.nhacungcap.list')->with(['flash_level'=>'success','flash_message'=>'Có lô hàng thuộc nhà cung cấp này không thể xóa!!!']);
        }
