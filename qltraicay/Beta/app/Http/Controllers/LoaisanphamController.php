...
class LoaisanphamController extends Controller
{
	...
	public function getDelete($id)
	{
		...
		$sanpham = DB::table('sanpham')->select('id')->where('loaisanpham_id',$id)->get();
        foreach ($sanpham as $key => $val) {
            $khmai[] = $val->id;
        }
		if (!empty($khmai)) {
			return redirect()->route('admin.loaisanpham.list')->with(['flash_level'=>'success','flash_message'=>'Có sản phẩm thuộc loại này không thể xóa!!!']);
        }
